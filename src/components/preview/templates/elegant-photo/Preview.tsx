...
/**
 * Elegant Photo Template
 * Two-column layout with dark sidebar (photo, name, contact, skills) and main content area.
 */
export function ElegantPhotoPreview() {
  ...
  const renderSidebarHeader = () => {
    ...
    return (
      <div className="mb-6 flex flex-col items-center">
        ...
        <p className="text-white/80 mb-4 text-center" style={{ fontSize: theme.titleFontSize - 2 }}>
          {data.title}
        </p>
        <div className="space-y-2 text-white/90 w-full" style={{ fontSize: theme.fontSize - 1 }}>
          {data.email && (
            ...
            <div key={category.id}>
              ...
              <div className="flex flex-wrap gap-1.5">
                {category.skills.map((skill, idx) => (
                  <span key={idx} className="px-2 py-0.5 text-xs rounded bg-white/20 text-white">{skill}</span>
                ))}
              </div>
            </div>
          ))}
      ...
      <div className="mb-5">
        ...
        <div className="flex flex-wrap gap-1.5">
          {data.items.map((item, idx) => (
            <span key={idx} className="px-2 py-0.5 text-xs rounded bg-white/20 text-white">{item}</span>
          ))}
        </div>
      </div>
    );
