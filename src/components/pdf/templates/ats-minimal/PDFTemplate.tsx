        ...
        {data.items.map((item) => (
          <View key={item.id} style={{ marginBottom: 12 }}>
            <Text style={styles.itemTitle}>
              {item.name}
              {item.link && <Text style={{ fontWeight: 400, color: "#9CA3AF" }}> | {item.link}</Text>}
            </Text>
      ...
      <View>
        ...
        {data.items.map((item) => (
          <View key={item.id} style={{ marginBottom: 8 }}>
            ...
            {item.email && <Text style={{ color: "#9CA3AF", fontSize: theme.fontSize - 1 }}>{item.email}</Text>}
